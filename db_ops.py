    ...
    def get_match_num(self, constraints, return_entry=False):
        ...
        for domain in all_domains:
            ...
            if domain in db_domains and constraints.get(domain):
                if 'food' in constraints[domain] and constraints[domain]['food']=='portugese':
                    constraints[domain]['food']='portuguese'
    ...
    def queryJsons(self, domain, constraints, exactly_match=True, return_name=False):
        ...
        if 'name' in constraints:
            for db_ent in self.dbs[domain]:
                if 'name' in db_ent:
                    ...
                    dbn = db_ent['name']
                    dbn = dbn.replace(' \'s', 's') # our vocabulary does not contain 's
                    dbn = dbn.replace('\'s', 's')
                    if cons in dbn or dbn in cons:# exact match is not necessary, for instance, the museum and museum
        ...
        for db_ent in self.dbs[domain]:
            ...
            for s, v in constraints.items():
                if s=='price' and v in ['moderate', 'cheap', 'expensive']:
                    s='pricerange'
                if s == 'name':
                    continue
